"""Build 01_recommendation_build.ipynb in the five Module 6 teaching stages.
...
SKELETON. The scaffolding, the cell helpers and the five stage headings are in
place; the stage content is not written yet. Each stage below carries a marked
block where its cells go. Adding content means adding ``md(...)`` and
``code(...)`` calls inside those blocks and nothing else - the header, the
helpers and the writer at the bottom are finished.
"""
