...
NOTEBOOK = PROJECT_DIR / "01_procedure_build.ipynb"
BACKUP_DIR = PROJECT_DIR / "backup"
OUTPUT = BACKUP_DIR / "01_procedure_build.html"
...
def main() -> None:
    ...
    for cell in notebook.cells:
        if cell.cell_type == "code" and any(
            ...
            raise SystemExit("Disable progress widgets and re-execute before creating offline HTML.")
    ...
    # Count actual render calls, not the string "Plotly.newPlot" - the inlined
    # plotly bundle mentions it once inside an error message.
    figures = len(re.findall(r"\)\)\s*\{\s*Plotly\.newPlot\(", html))
    ...
    print(f"Plotly figures embedded : {figures} (expected {EXPECTED_FIGURES})")
    ...
    if figures != EXPECTED_FIGURES:
        raise SystemExit(
            f"Backup renders {figures} figures; the notebook has {EXPECTED_FIGURES}. "
            "Re-execute the notebook before exporting."
        )
